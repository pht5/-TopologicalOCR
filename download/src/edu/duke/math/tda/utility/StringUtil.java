...
public class StringUtil {
	...
	private static boolean validateRequiredData() throws Exception {
	    ...
	    // Validate:
	    settingNameCanonical = TDA.SETTING_PROJECT;
	    settingNameDescriptive = TDA.SETTING_PROJECT_DESCR;
	    settingNameForDisplay = TDA.SETTING_PROJECT_DISP;
	    ...
	    settingItem = processData.processSetting( settingNameCanonical, 
	            ...
	            TDA.DATA_SETTINGDEFAULTVALUE_EMPTY );
	    
	    // Validate:
	    settingNameCanonical = TDA.SETTING_USER;
	    settingNameDescriptive = TDA.SETTING_USER_DESCR;
	    settingNameForDisplay = TDA.SETTING_USER_DISP;
	    settingDataType = TDA.VALIDATION_DATATYPE_STRING;
	    validationType = TDA.VALIDATIONTYPE_OPTIONAL;
	    settingItem = processData.processSetting( settingNameCanonical, 
	            settingNameDescriptive,
	    ...
	    // Validate:
	    settingNameCanonical = TDA.SETTING_DATASET;
	    settingNameDescriptive = TDA.SETTING_DATASET_DESCR;
	    settingNameForDisplay = TDA.SETTING_DATASET_DISP;
	    settingDataType = TDA.VALIDATION_DATATYPE_STRING;
	    validationType = TDA.VALIDATIONTYPE_OPTIONAL;
	    settingItem = processData.processSetting( settingNameCanonical, 
	            settingNameDescriptive,
	    ...
	    // Validate:
	    settingNameCanonical = TDA.SETTING_NOTES;
	    settingNameDescriptive = TDA.SETTING_NOTES_DESCR;
	    settingNameForDisplay = TDA.SETTING_NOTES_DISP;
	    settingDataType = TDA.VALIDATION_DATATYPE_STRING;
	    validationType = TDA.VALIDATIONTYPE_OPTIONAL;
	    settingItem = processData.processSetting( settingNameCanonical, 
	            settingNameDescriptive,
	...
	public static String fillSpaces( 
		...
		String fillString;
		String blanks = TDA.FEEDBACK_SPACESFORTRIMMING;
	...
	public static String removeTrailingComment( final String _stringToProcess ) {
	    ...
	    int charPosition = processedString.indexOf( TDA.DEFAULT_COMMENTINDICATOR );
	...
	public static StringBuffer formatRightLeftJustified( 
		...
		String blankSpaces = TDA.FEEDBACK_SPACESFORTRIMMING;
	...
	public static StringBuffer listSettings( final Properties _settingsToList ) {
	    ...
	    settingsString.append( TDA.FEEDBACK_NEWLINE );
    ...
    /**
     * Compiles the setting values that we group as the "Tda signature".
     *
     * @return StringBuffer The collect "job signature".
     */
    public static StringBuffer getTdaSignature() {
        
        StringBuffer tdaSignature = new StringBuffer( TDA.BUFFERLENGTH_STAT );	    

	    // Adding a little extra formatting
		String newLinePlusPrefix = TDA.FEEDBACK_NEWLINE + TDA.FEEDBACK_DASH + 
				TDA.FEEDBACK_SPACE;
		String linePostfix = TDA.FEEDBACK_SPACE + TDA.FEEDBACK_DASH;
		int lineLength = TDA.FEEDBACK_LINELENGTH;
		String HeaderInfo;
		
		tdaSignature.append( StringUtil.formatRightLeftJustified(
		        newLinePlusPrefix, TDA.APPLICATION_NAME, 
		        TDA.APPLICATION_NAME_LONG + linePostfix, 
		        null, lineLength ) );

		HeaderInfo = TDA.RELEASE + TDA.FEEDBACK_SPACE + 
			TDA.APPLICATION_VERSIONNUMBER;
		tdaSignature.append( StringUtil.formatRightLeftJustified( 
		        newLinePlusPrefix, HeaderInfo, 
		        TDA.APPLICATION_VERSIONDATE + linePostfix, 
		        null, lineLength ) );

//		HeaderInfo = TDA;
//		tdaSignature( StringUtil.formatRightLeftJustified( 
//		        newLinePlusPrefix, HeaderInfo, 
//		        linePostfix, 
//		        null, lineLength ) );

		HeaderInfo = TDA.RELEASE_COPYRIGHT;
		tdaSignature.append( StringUtil.formatRightLeftJustified( 
		        newLinePlusPrefix, HeaderInfo, 
		        linePostfix, 
		        null, lineLength ) );

		HeaderInfo = TDA.RELEASE_ALLRIGHTS;
		tdaSignature.append( StringUtil.formatRightLeftJustified( 
		        newLinePlusPrefix, HeaderInfo, 
		        linePostfix, 
		        null, lineLength ) );        
    ...
    public static StringBuffer getJobSignature( final Settings _processData ) {
        ...
        StringBuffer jobSignature = new StringBuffer( TDA.BUFFERLENGTH_STAT );
		...
		jobSignature.append( StringUtil.formatRightLeftJustified( 
		        newLinePlusPrefix, 
		        TDA.SETTING_PROJECT_DESCR + TDA.FEEDBACK_COLON,
		...
		jobSignature.append( StringUtil.formatRightLeftJustified( 
		        newLinePlusPrefix, 
		        TDA.SETTING_USER_DESCR + TDA.FEEDBACK_COLON,
		        strSettingChoice, null, lineLength ) );
		...
		jobSignature.append( StringUtil.formatRightLeftJustified( 
		        newLinePlusPrefix, 
		        TDA.SETTING_DATASET_DESCR + TDA.FEEDBACK_COLON,
		        strSettingChoice, null, lineLength ) );
		...
		jobSignature.append( StringUtil.formatRightLeftJustified( 
		        newLinePlusPrefix, 
		        TDA.SETTING_NOTES_DESCR + TDA.FEEDBACK_COLON,
		        strSettingChoice, null, lineLength ) );
    ...
    public static String getReportFileNameMT( String _reportFileName, 
        ...
        strToken = TDA.DATA_THREADID_TOKEN.toLowerCase();
        defaultPrefix = TDA.SETTING_FILENAMEPREFIXFORTHREADS_DEFAULT;
    ...
    public static StringBuffer formatElapsedTime(
		...
		if ( _decimalsToDisplay < 0 ) {
		    _decimalsToDisplay = TDA.FEEDBACK_NUMBEROFDECIMALSINTIMEDISPLAY;
		}
		...
		if ( _formatFlag == TDA.FEEDBACK_OPTION_TIMEFORMAT_MIXED  ) {
			
		    // Note: the cutoffs when using different formats are somewhat arbitrary
            if ( _elapsedTime > TDA.FEEDBACK_DEFAULT_CUTOFF_TO_DAYS * MILLISECSPERDAY ) {
                ...
                formattedElapsedTime.append( TDA.FEEDBACK_DEFAULT_TIME_DAYS );
            }
            else if ( _elapsedTime > TDA.FEEDBACK_DEFAULT_CUTOFF_TO_HOURS * MILLISECSPERHOUR ) {
				...
				formattedElapsedTime.append( dblElapsedTime );
				formattedElapsedTime.append( " " );
				formattedElapsedTime.append( TDA.FEEDBACK_DEFAULT_TIME_HOURS );
			}
			else if ( _elapsedTime > TDA.FEEDBACK_DEFAULT_CUTOFF_TO_MINUTES * 
			        MILLISECSPERMINUTE ) {
				...
				formattedElapsedTime.append( dblElapsedTime );
				formattedElapsedTime.append( " " );
				formattedElapsedTime.append( TDA.FEEDBACK_DEFAULT_TIME_MINUTES );
			}
			else if ( _elapsedTime > TDA.FEEDBACK_DEFAULT_CUTOFF_TO_SECONDS * 
			        MILLISECSPERSECOND ) {
				...
				formattedElapsedTime.append( dblElapsedTime );
				formattedElapsedTime.append( " " );
				formattedElapsedTime.append( TDA.FEEDBACK_DEFAULT_TIME_SECONDS );
			}
			else {
				...
				formattedElapsedTime.append( " " );
				formattedElapsedTime.append( TDA.FEEDBACK_DEFAULT_TIME_MILLISECS );
			}
		}
        else if ( _formatFlag == TDA.FEEDBACK_OPTION_TIMEFORMAT_IN_D ) {           
        ...
        else if ( _formatFlag == TDA.FEEDBACK_OPTION_TIMEFORMAT_IN_H ) {           
		...
		else if ( _formatFlag == TDA.FEEDBACK_OPTION_TIMEFORMAT_IN_M ) {		    
		...
		else if ( _formatFlag == TDA.FEEDBACK_OPTION_TIMEFORMAT_IN_S ) {
		...
		else if ( _formatFlag == TDA.FEEDBACK_OPTION_TIMEFORMAT_IN_MS ) {
    ...
    public static String timeStamp( 
        ...
        try {
            ...
            if ( _defaultTimeStampFormat == null || 
                ...
                _defaultTimeStampFormat = TDA.DEFAULT_TIMESTAMP;
            }
            ...
            // a warning message
            _processData.addToWarnings( new TdaError( 
                "The format for the supplied time stamp ('" +
                _timeStampFormat + "') for setting '" +
                _settingName + "' is not valid. " +
                TDA.APPLICATION_NAME + " will use the supplied default value (='" +
                _defaultTimeStampFormat + "') instead.",
	            TDA.ERRORTYPE_ALERT_CORRECTEDCHOICE,
                _settingName,
        		null ) );
            ...
            try {
                ...
                _processData.addToWarnings( new TdaError( 
	                ...
	                _timeStampFormat + "') for setting '" +
                    _settingName + "' is not valid. " +
	                TDA.APPLICATION_NAME + " will use the system default value (='" +
	                TDA.DEFAULT_TIMESTAMP + "') instead.",
		            TDA.ERRORTYPE_ALERT_CORRECTEDCHOICE,
                    _settingName,
            		null ) );
                
                formatter = new SimpleDateFormat( TDA.DEFAULT_TIMESTAMP );
		        cachedTimeStamp = formatter.format(date);
            }
        }
    ...
    public static String compileMemoryInfo( final String _info ) {
        ...
        if ( _info != null && !_info.equals("") ) {
            ...
            memoryInfo.append( TDA.FEEDBACK_SPACE );
        }
        
        memoryInfo.append( TDA.APPLICATION_NAME + " is using " + (totalMem - freeMem)/(1024*1024) + 
                " (out of an available " + maxMem/1024/1024 + ") MB of memory" );
